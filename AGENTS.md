...
- `topics-ai/hermes/install.sh` (Docker-free phase) installs NousResearch Hermes Agent into `~/.hermes/` and wires **everything** idempotently: core (`--skip-setup`), model routing → LiteLLM custom endpoint (`http://localhost:4000/v1`, `LITELLM_KEY` master key; `deepseek-v4-flash-deepseek` default for the main loop and text-only aux tasks `compression`/`title_generation`/`web_extract`/`session_search`, with `deepseek-v4-pro-deepseek` reserved for delegated subagents via `delegation.model`), `agent.reasoning_effort = none` (DeepSeek via LiteLLM rejects the OpenAI `reasoning_effort` param), the Phoenix MCP server (`mcp_servers.phoenix.url` = `http://localhost:6006/mcp`), the `backsearch` plugin, the `official/devops/docker-management` skill, the community `adhd-assistant` and `i-have-adhd` skills (clawhub `@tobeyrebecca/adhder-assistant` + GitHub `ayghri/i-have-adhd` — the user-requested exceptions to the no-community rule), the self-evolution utility, and the **native desktop app** (Electron, built in place via `hermes desktop --build-only` + macOS `--setup-tcc-identity` so TCC grants survive rebuilds). The desktop app is the same agent as the CLI — same config/keys/sessions/skills/memory — and the canonical self-updating build lives under `apps/desktop/release/` (a separate `/Applications/Hermes.app` from the website installer is NOT managed by this topic).
...
- **AI skill tracking lives in `topics-ai/skills/manifest`** — one **tool-agnostic** registry for every agent skill this machine uses (columns: `<name> <source> <tools> <identifier>`), consumed by `topics-ai/skills/install.sh` and by `topics-ai/hermes/install.sh` section 5. Sources: `builtin` (ships with Hermes → verified present, **never** installed), `official`/`clawhub`/`url` (installed idempotently by Hermes), `local` (authored in `topics-ai/skills/library/`, shared with **every** tool). **Whenever you install or author a skill, add a row here** so a fresh bootstrap reproduces it. Live source: `hermes skills list` (column `source`); `hermes skills snapshot export -` dumps the non-builtin set as JSON.
- **Cross-tool skills**: `~/.agents/skills` is a symlink to `topics-ai/skills/library/` — the one path OpenCode scans natively (`~/.agents/skills`, `~/.config/opencode/skills`, `~/.claude/skills`) and Hermes reads via `skills.external_dirs`. Upstream/hub skills are deliberately **not** vendored into this repo; only `local` skills are shared. A skill's frontmatter `name` **must equal its directory name** or OpenCode silently ignores it (Hermes does not enforce this — `adhder-assistant` is the known offender).
