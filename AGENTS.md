...
- `./install.sh` runs topic installers in **two phases after Homebrew/Bootstrap**:
  ...
  3. Topics that declare `NEEDS_DOCKER=1` (e.g. `litellm/install.sh`) — these run only after the gate succeeded.
...
- **Secret keys live only in `.local/.env.local`** → mirrored to `~/.hermes/.env` by `hermes/install.sh`. Only optional key today: `OPENREWARD_API_KEY` (backsearch). Never edit `~/.hermes/.env` directly; add the var to `.env.example`/`.env.local` and re-run the topic installer.
...
- Runtime record of everything installed: `~/.hermes/MANIFEST.md` (source: `hermes/MANIFEST.md`).
